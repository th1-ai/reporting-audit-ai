---
name: reporting-audit-ai
description: Run Reporting & Audit AI ("The Auditor") — Two siblings.. Use when the user asks to run the agent, check what is waiting for review, approve or reject a draft, or asks how the agent is doing. Trigger phrases: "run The Auditor", "/reporting-audit-ai", "check the queue", "what is waiting for me", "approve that draft".
---

# Reporting & Audit AI

Runs Reporting & Audit AI and works its review queue. Everything happens from the repo
...
make run ARGS="--dry-run"       # compute everything, no business data written
```
